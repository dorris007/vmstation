...
class XPertMailer
{
	...
	function clear_attach($clearatt = 3){
		if(is_int($clearatt) && ($clearatt == 1 || $clearatt == 2 || $clearatt == 3)){
			...
			elseif($clearatt == 2) $this->_attc2nd = array();
			elseif($clearatt == 3){
				$this->_attc1st = array();
				$this->_attc2nd = array();
			}
		}elseif(PRINT_ERROR) trigger_error("Invalid parameter value on class XPertMailer->clear_attach()", 512);
	}
	...
	function _is_connection($connection){
	...
	function _sendtoip($ip, $toarr, $relay = false){
		...
		if($connect = fsockopen($ssl.$ip, $pnm, $err_num, $err_msg, $this->_timeout)){
			...
			$loop = 0;
			stream_set_timeout($connect, $this->_timeout);
			while(!feof($connect)){
				$loop++;
				$rcv = fgets($connect, 1024);
				if(substr($rcv, 0, 4) != "220-" || $loop === 99) break;
			}
			...
			if($set){
				...
				if($relay && $this->_authsets){
					...
					if($this->_is_connection($connect)){
						...
						while(!feof($connect)){
							...
							$rcv = fgets($connect, 1024);
							$getinfo .= $rcv;
							if(substr($rcv, 0, 4) != "250-" || $loop === 99) break;
						}
						...
						$this->response = "Response C0 error: Invalid resource connection";
						$set = false;
					}
					...
					if(!$authreq || $authreq == "login"){
						...
						if($set){
							if($this->_is_connection($connect)){
								...
								$this->response = "Response C1 error: Invalid resource connection";
								$set = false;
							}
						}
						if($set){
							if($this->_is_connection($connect)){
								...
								$this->response = "Response C2 error: Invalid resource connection";
								$set = false;
							}
						}
						if($set){
							if($this->_is_connection($connect)){
								...
								$this->response = "Response C3 error: Invalid resource connection";
								$set = false;
							}
						}
						...
						if($set){
							if($this->_is_connection($connect)){
								...
								$this->response = "Response C4 error: Invalid resource connection";
								$set = false;
							}
						}
					...
					if($this->_is_connection($connect)){
						...
						$this->response = "Response C5 error: Invalid resource connection";
						$set = false;
					}
			...
			if($set){
				if($this->_is_connection($connect)){
					...
					$this->response = "Response C6 error: Invalid resource connection";
					$set = false;
				}
			}
				...
				foreach($toarr as $arrval){
					if($set){
						if($this->_is_connection($connect)){
							...
							$this->response = "Response C7 error: Invalid resource connection";
							$set = false;
						}
					}else break;
				}
			...
			if($set){
				if($this->_is_connection($connect)){
					...
					$this->response = "Response C8 error: Invalid resource connection";
					$set = false;
				}
			}

			if($set){
				foreach($this->_msgsplit as $partmsg){
					if($this->_is_connection($connect)) fputs($connect, $partmsg);
					else{
						$this->response = "Response C9 error: Invalid resource connection";
						$set = false;
						break;
					}
				}
				if($set){
					if($this->_is_connection($connect)){
						...
						$this->response = "Response C10 error: Invalid resource connection";
						$set = false;
					}
				}
				if($set){
			...
			if($this->_is_connection($connect)) fclose($connect);
	...
	function _sendtohost($host, $detarr, $toaddh, $hccstr, $hbccstr, $dtmsg, $tpmime, $hrelay = false){
		...
		if(strtolower($host) == "localhost"){
			...
			if($this->_is_ip($host)) return $this->_sendtoip($host, $detarr, $hrelay);
				...
				}else $this->response = "Hostname '".$host."' doesn't have an IP address !";
